...
# from https://arxiv.org/pdf/2106.05081
...
parser.add_argument(
    "--dataset", default="diginetica", help="diginetica/Nowplaying/Tmall"
)
...
def main():
    ...
    # create a data split: train and validation/test
    if opt.validation:
    ...
    # read the adjacency matrix and the number of neighbors
    adj = pickle.load(
        open("datasets/" + opt.dataset + "/adj_" + str(opt.n_sample_all) + ".pkl", "rb")
    )
    num = pickle.load(
        open("datasets/" + opt.dataset + "/num_" + str(opt.n_sample_all) + ".pkl", "rb")
    )
